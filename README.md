...
:root{
  --green:#6DD400;
  --green-dim:rgba(109,212,0,0.18);
  --green-text:rgba(109,212,0,0.75);
  --card:#0D1E0D;
  --border:rgba(109,212,0,0.18);
  --text:#fff;
...
.tab{flex:1;padding:10px 4px;border-radius:7px;font-size:12px;font-family:'Roboto',sans-serif;font-weight:700;cursor:pointer;border:none;background:transparent;color:rgba(255,255,255,0.6);transition:all 0.2s;letter-spacing:0.3px;}
...
.filter-btn{padding:5px 14px;border-radius:20px;font-size:11px;font-family:'Roboto',sans-serif;font-weight:700;cursor:pointer;border:1px solid rgba(109,212,0,0.25);background:transparent;color:rgba(255,255,255,0.6);transition:all 0.15s;}
...
.obs-text{font-size:10px;color:rgba(255,255,255,0.45);line-height:1.4;max-width:260px;}
.name-cell{font-weight:700;color:#fff;}
.loc-cell{color:rgba(255,255,255,0.5);font-size:11px;}
.contact-cell{font-size:11px;color:rgba(255,255,255,0.75);}
.phone-cell{font-size:11px;color:var(--green-text);}

/* BADGES */
.badge{display:inline-flex;align-items:center;gap:4px;padding:3px 9px;border-radius:20px;font-size:10px;font-weight:700;font-family:'Roboto',sans-serif;white-space:nowrap;}
...
.badge.otro{background:rgba(160,160,160,0.12);color:#A0A0A0;border:1px solid rgba(160,160,160,0.25);}
...
.step-label{font-size:11px;color:rgba(255,255,255,0.6);font-family:'Roboto',sans-serif;flex:1;}
.step-val{font-size:10px;font-family:'Roboto',sans-serif;text-align:right;max-width:150px;}
...
.assets-table td{padding:9px 10px;font-size:12px;color:#fff;border-bottom:1px solid rgba(109,212,0,0.06);font-family:'Roboto',sans-serif;vertical-align:middle;}
...
.funnel-bar-fill{height:100%;border-radius:6px;display:flex;align-items:center;padding-left:10px;font-size:11px;font-weight:700;font-family:'Roboto',sans-serif;}
.funnel-label{font-size:11px;color:rgba(255,255,255,0.55);width:120px;text-align:right;font-family:'Roboto',sans-serif;}
...
/* FAB */
.fab{position:fixed;bottom:28px;right:28px;background:var(--green);color:#0A1A0A;border:none;border-radius:50%;width:54px;height:54px;font-size:24px;cursor:pointer;box-shadow:0 4px 20px rgba(109,212,0,0.3);display:flex;align-items:center;justify-content:center;transition:transform 0.2s;}
.fab:hover{transform:scale(1.08);}

/* CONFIRM ASSET EDIT */
.asset-cell-edit{cursor:pointer;border-radius:4px;padding:2px 6px;}
...
<div id="panel-pipeline" class="panel active">
  ...
  <div class="scroll-table">
    <table class="prospect-table">
      <thead><tr>
        <th>#</th><th>Restaurante</th><th>Ubicación</th><th>Contacto</th><th>Teléfono</th><th>Status</th><th>Observaciones</th><th>Acción</th>
      </tr></thead>
...
<div id="panel-assets" class="panel">
  ...
  <div class="scroll-table">
    <table class="assets-table">
      <thead><tr>
        <th>#</th><th>Restaurante</th><th>Contacto</th><th>Logo</th><th>Mención Producto</th><th>Mención Marca</th><th>Stories IG</th><th>% Completo</th>
      </tr></thead>
...
<div id="panel-overview" class="panel">
  <div class="overview-2col">
    <div class="chart-wrap">
      <div class="section-title">Funnel de negociación</div>
      <div id="funnel-chart"></div>
    </div>
    <div class="chart-wrap">
      <div class="section-title">Estado de firmas</div>
      <div id="firma-chart"></div>
      <div style="height:14px;"></div>
      <div class="section-title">Cobertura de assets</div>
      <div id="assets-summary"></div>
    </div>
  </div>
  <div class="chart-wrap">
    <div class="section-title">Distribución por zona</div>
    <div id="zona-chart"></div>
  </div>
</div>
...
<div class="modal-overlay" id="modal">
  <div class="modal">
    <h2 id="modal-title">Agregar / Editar Prospecto</h2>
    <div class="form-2col">
      <div class="form-row">
        <label class="form-label">Restaurante</label>
        <input class="form-input" id="f-rest" type="text" placeholder="Nombre del restaurante"/>
      </div>
      <div class="form-row">
        <label class="form-label">Ubicación</label>
        <input class="form-input" id="f-ubi" type="text" placeholder="Zona o dirección"/>
      </div>
    </div>
    <div class="form-2col">
      <div class="form-row">
        <label class="form-label">Contacto</label>
        <input class="form-input" id="f-cont" type="text" placeholder="Nombre del contacto"/>
      </div>
      <div class="form-row">
        <label class="form-label">Teléfono</label>
        <input class="form-input" id="f-tel" type="text" placeholder="6000-0000"/>
      </div>
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
      ...
      <textarea class="form-textarea" id="f-obs" placeholder="Notas clave del prospecto..."></textarea>
    </div>
    <div class="btn-row">
...
let prospects = JSON.parse(localStorage.getItem('tvmax_prospects') || 'null') || [
  {r:'Boho',u:'Río Abajo',c:'Franklyn Robinson',t:'6093-7160',s:'cerrado',obs:'Fanfest exterior planeado con I Star Panamá. Pendiente logo y materiales.'},
  ...
  {r:'Club Unión Española',u:'Carrasquilla',c:'Luis Arias',t:'6030-0777',s:'cerrado',obs:'Firma 26 mayo. No comunicar como bar, sino como club deportivo.'},
  {r:'La Manzana',u:'Santa Ana',c:'Edwards Molla',t:'6183-4712',s:'sin-resp',obs:'Evaluando propuestas vs Cable Onda Sports y RPC.'},
  {r:'Grill 50',u:'San Francisco',c:'Stefany García',t:'',s:'pendiente',obs:'Seguimiento dado el 25 de mayo.'},
  {r:'La Rana Dorada',u:'Ubicaciones varias',c:'Blanca y Hernán',t:'',s:'otro',obs:'Firmaron con otra plataforma.'},
  {r:'The Yard',u:'San Francisco',c:'Jorge Castro',t:'6611-8931',s:'cerrado',obs:'Firma 12 mayo. Pase en vivo realizado. Cuidar tema Balboa.'},
  ...
  {r:'Feroz',u:'Ciudad del Saber',c:'Cesar Conto',t:'6090-7564',s:'cerrado',obs:'Firma 18 mayo. Posible ceviche temático.'},
  {r:'Taberna 21',u:'Cangrejo',c:'—',t:'',s:'sin-resp',obs:'Sin contacto registrado.'},
  {r:'Bingo 90',u:'Obarrio',c:'Stephanie',t:'',s:'pendiente',obs:'34 salas en ciudad e interior. Esperando propuesta formal.'},
  {r:'Pintas Spot',u:'Calle 50',c:'Dainubys Sarmiento',t:'6647-6772',s:'pendiente',obs:'Seguimiento 25 mayo. No han respondido.'},
  ...
  {r:'X3M',u:'Tumba Muerto',c:'Curtis',t:'6264-6589',s:'cerrado',obs:'Documento enviado. Pendiente fecha de firma.'},
  ...
  {r:'Austin Mamas',u:'Corozal y Costa Este',c:'Sebastián',t:'6898-6073',s:'pendiente',obs:'12 pantallas en Corozal. Resistencia a cupones.'},
  ...
  {r:'La Mandolina',u:'Santiago',c:'Ana Laura Virzi',t:'',s:'cerrado',obs:'Firma 16 mayo. Muy activos con Veracruz United.'},
  {r:'Efímero',u:'—',c:'Fady',t:'6450-5500',s:'sin-resp',obs:'Sin respuesta.'},
  {r:'Autódromo',u:'—',c:'Liz Navarro',t:'6747-8083',s:'sin-resp',obs:'Pendiente nueva fecha.'},
  ...
  {r:'Grupo Maito',u:'—',c:'Mary De Luca',t:'6639-5230',s:'cerrado',obs:'El Callejón Casco. Pendiente documento.'},
  {r:'Cervecería Industrial',u:'La Chorrera',c:'Marcos',t:'6054-7060',s:'pendiente',obs:'Pendiente respuesta.'},
...
let confirmed = JSON.parse(localStorage.getItem('tvmax_confirmed') || 'null') || [
  ...
  {n:'Tacos La Neta',l:'Brisas / Casco',c:'—',doc:'pend',firma:'pend',pase:'na',contenido:'pend'},
  {n:'Asaito',l:'Brisas',c:'—',doc:'pend',firma:'pend',pase:'na',contenido:'pend'},
...
let assets = JSON.parse(localStorage.getItem('tvmax_assets') || 'null') || confirmed.map(c=>({n:c.n,contact:c.c,logo:'pend',mp:'pend',mm:'pend',st:'pend'}));
// Fix logos already received per original data
['Boho','Minuto7','5 to 5 Hilton','The Yard','X3M','PESKITO','Canal House','Feroz','Club Unión Española','Dorado Mall','Huekito Mexicano','Central','5inco','La 10 Colón','The Win','Donde Arturo','El Callejón','Tacos La Neta','Asaito','Wings Famous Grill','Brisas Capital'].forEach(name=>{
  const a = assets.find(x=>x.n===name);
  if(a && a.logo==='pend') a.logo='recibido';
});
if(!localStorage.getItem('tvmax_assets')) saveToStorage();

function saveToStorage(){
  localStorage.setItem('tvmax_prospects', JSON.stringify(prospects));
  localStorage.setItem('tvmax_confirmed', JSON.stringify(confirmed));
  localStorage.setItem('tvmax_assets', JSON.stringify(assets));
}

function updateMetrics(){
  const cerrados = prospects.filter(p=>p.s==='cerrado').length;
  const proceso = prospects.filter(p=>p.s==='pendiente').length;
  ...
  document.getElementById('m-pend').textContent = proceso;
...
function renderProspects(){
  const rows = prospects.map((p,i)=>({...p,_i:i})).filter(p=>{
    const matchF = currentFilter==='all'||p.s===currentFilter;
    const q = currentSearch.toLowerCase();
    const matchS = !q||(p.r+p.u+p.c).toLowerCase().includes(q);
    return matchF&&matchS;
  });
  document.getElementById('prospect-body').innerHTML = rows.map((p,idx)=>`
    <tr>
      <td style="color:rgba(109,212,0,0.5);font-size:10px;">${idx+1}</td>
      <td class="name-cell">${p.r}</td>
      <td class="loc-cell">${p.u||'—'}</td>
      <td class="contact-cell">${p.c||'—'}</td>
      <td class="phone-cell">${p.t||'—'}</td>
      <td>${badgeHTML(p.s)}</td>
      <td class="obs-text">${p.obs}</td>
      <td><button onclick="openEditModal(${p._i})" style="background:rgba(109,212,0,0.1);border:1px solid rgba(109,212,0,0.25);color:#6DD400;border-radius:6px;padding:4px 10px;font-size:10px;font-weight:700;cursor:pointer;font-family:'Roboto',sans-serif;">Editar</button></td>
    </tr>`).join('');
}

function renderConfirmed(){
  document.getElementById('confirm-grid').innerHTML = confirmed.map((c,i)=>{
    const docDone=c.doc==='enviado';
    const firmaDone=c.firma!=='pend'&&c.firma!=='TBD';
    const paseDone=c.pase!=='na'&&c.pase!=='pend';
    const contDone=c.contenido==='done';
    const firmaLabel=c.firma==='pend'?'Pendiente TVMAX':c.firma==='TBD'?'TBD - Esperando local':('✓ Realizada el '+c.firma);
    return `<div class="confirm-card">
      <h3>${c.n}</h3>
      <div class="confirm-loc">📍 ${c.l} &nbsp;|&nbsp; 👤 ${c.c}</div>
      <div class="step-row"><div class="step-dot ${docDone?'done':'pend'}"></div><span class="step-label">Documento de alianza</span><span class="step-val ${docDone?'done':'pend'}">${docDone?'Enviado':'Pendiente cliente'}</span></div>
      <div class="step-row"><div class="step-dot ${firmaDone?'done':'pend'}"></div><span class="step-label">Firma simbólica</span><span class="step-val ${firmaDone?'done':'pend'}" style="font-size:9px;">${firmaDone?('✓ '+c.firma):firmaLabel}</span></div>
      <div class="step-row"><div class="step-dot ${paseDone?'done':c.pase==='na'?'na':'pend'}"></div><span class="step-label">Pase en vivo</span><span class="step-val ${paseDone?'done':c.pase==='na'?'na':'pend'}">${paseDone?'✓ Realizado':c.pase==='na'?'N/A':'Pendiente'}</span></div>
      <div class="step-row"><div class="step-dot ${contDone?'done':'pend'}"></div><span class="step-label">Contenido firma</span><span class="step-val ${contDone?'done':'pend'}">${contDone?'✓ Listo':'Pendiente'}</span></div>
      <div style="margin-top:8px;display:flex;gap:6px;">
        <button onclick="editConfirmed(${i})" style="flex:1;background:rgba(109,212,0,0.08);border:1px solid rgba(109,212,0,0.2);color:#6DD400;border-radius:6px;padding:5px;font-size:10px;font-weight:700;cursor:pointer;font-family:'Roboto',sans-serif;">Editar estado</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderAssets(){
  document.getElementById('assets-body').innerHTML = assets.map((a,i)=>{
    ...
    const labels=['Mención\nProducto','Mención\nMarca','Stories\nIG'];
    const done=fields.filter(f=>a[f]==='recibido').length;
    const pct=Math.round((done/4)*100);
    const cellStyle=(v)=>v==='recibido'?'color:#6DD400;cursor:pointer;text-align:center;':'color:#F5A623;cursor:pointer;text-align:center;';
    const cellText=(v)=>v==='recibido'?'✅':'⏳';
    return `<tr>
      ...
      <td class="asset-cell-edit" style="${cellStyle(a.logo)}" onclick="toggleAsset(${i},'logo')" title="Clic para cambiar">${cellText(a.logo)}</td>
      <td class="asset-cell-edit" style="${cellStyle(a.mp)}" onclick="toggleAsset(${i},'mp')" title="Clic para cambiar">${cellText(a.mp)}</td>
      <td class="asset-cell-edit" style="${cellStyle(a.mm)}" onclick="toggleAsset(${i},'mm')" title="Clic para cambiar">${cellText(a.mm)}</td>
      <td class="asset-cell-edit" style="${cellStyle(a.st)}" onclick="toggleAsset(${i},'st')" title="Clic para cambiar">${cellText(a.st)}</td>
      <td style="min-width:70px;">
        <div style="font-size:11px;font-weight:700;color:${pct===100?'#6DD400':pct>50?'#F5A623':'#E53E3E'};">${pct}%</div>
        <div class="pct-bar"><div class="pct-fill" style="width:${pct}%;background:${pct===100?'#6DD400':pct>50?'#F5A623':'#E53E3E'};"></div></div>
      </td>
    </tr>`;
  }).join('');
}

function toggleAsset(i,field){
  assets[i][field] = assets[i][field]==='recibido'?'pend':'recibido';
  saveToStorage();
  renderAssets();
}
...
function renderOverview(){
  ...
  const funnelData=[
    {l:'Aliados cerrados',v:cerrados,color:'#6DD400',textColor:'#0A1A0A'},
    {l:'En negociación',v:pend,color:'#F5A623',textColor:'#0A1A0A'},
    {l:'Sin respuesta',v:sinResp,color:'#E53E3E',textColor:'#fff'},
    {l:'Perdidos / otro',v:otro,color:'rgba(160,160,160,0.6)',textColor:'#fff'},
  ];
  document.getElementById('funnel-chart').innerHTML=funnelData.map(f=>`
    <div class="funnel-item">
      <span class="funnel-label">${f.l}</span>
      <div class="funnel-bar-bg">
        <div class="funnel-bar-fill" style="width:${Math.round((f.v/total)*100)}%;background:${f.color};color:${f.textColor};">&nbsp;${f.v}</div>
      </div>
    </div>`).join('');

  const firmasDone=confirmed.filter(c=>c.firma!=='pend'&&c.firma!=='TBD').length;
  const firmasPend=confirmed.filter(c=>c.firma==='pend'||c.firma==='TBD').length;
  document.getElementById('firma-chart').innerHTML=`
    <div class="funnel-item">
      <span class="funnel-label" style="font-size:10px;">Realizadas</span>
      <div class="funnel-bar-bg" style="height:22px;"><div class="funnel-bar-fill" style="width:${Math.round((firmasDone/confirmed.length)*100)}%;background:#6DD400;color:#0A1A0A;font-size:11px;">&nbsp;${firmasDone}</div></div>
    </div>
    <div class="funnel-item">
      <span class="funnel-label" style="font-size:10px;">Pendientes</span>
      <div class="funnel-bar-bg" style="height:22px;"><div class="funnel-bar-fill" style="width:${Math.round((firmasPend/confirmed.length)*100)}%;background:#F5A623;color:#0A1A0A;font-size:11px;">&nbsp;${firmasPend}</div></div>
    </div>`;

  const logosDone=assets.filter(a=>a.logo==='recibido').length;
  const pct=Math.round((logosDone/assets.length)*100);
  document.getElementById('assets-summary').innerHTML=`
    <div style="font-size:12px;color:rgba(255,255,255,0.7);margin-bottom:5px;">Logos recibidos: <strong style="color:#6DD400;">${logosDone}/${assets.length}</strong></div>
    <div class="pct-bar" style="height:6px;"><div class="pct-fill" style="width:${pct}%;background:#6DD400;height:6px;"></div></div>
    <div style="font-size:11px;color:rgba(255,255,255,0.35);margin-top:6px;">Menciones y Stories: pendientes por parte de todos los aliados.</div>`;

  const ciudadCount=confirmed.filter(c=>!['Santiago','Chitré','Colón','La Chorrera'].some(z=>c.l.includes(z))).length;
  const interiorCount=confirmed.filter(c=>['Santiago','Chitré'].some(z=>c.l.includes(z))).length;
  const colonCount=confirmed.filter(c=>c.l.includes('Colón')).length;
  const oesteCount=confirmed.filter(c=>c.l.includes('Chorrera')).length;
  const zonas=[
    {l:'Ciudad de Panamá',v:ciudadCount,color:'rgba(109,212,0,0.7)'},
    {l:'Colón',v:colonCount,color:'rgba(99,179,237,0.7)'},
    {l:'Interior (Santiago/Chitré)',v:interiorCount,color:'rgba(245,166,35,0.7)'},
    {l:'Panamá Oeste',v:oesteCount,color:'rgba(160,160,160,0.6)'},
  ];
  document.getElementById('zona-chart').innerHTML=zonas.map(z=>`
    <div class="funnel-item">
      <span class="funnel-label">${z.l}</span>
      <div class="funnel-bar-bg">
        <div class="funnel-bar-fill" style="width:${Math.round((z.v/confirmed.length)*100)}%;background:${z.color};color:#0A1A0A;font-size:11px;">&nbsp;${z.v} aliados</div>
      </div>
    </div>`).join('');
}
...
function openAddModal(){
  ...
  document.getElementById('f-ubi').value='';
...
function editConfirmed(i){
  const c=confirmed[i];
  const docOpts=['enviado','pend'].map(v=>`<option value="${v}" ${c.doc===v?'selected':''}>${v==='enviado'?'✅ Enviado':'⏳ Pendiente cliente'}</option>`).join('');
  const firmaVal=prompt(`Firma simbólica de "${c.n}":\nEscribe la fecha realizada, "pend" si está pendiente, o "TBD":`,c.firma);
  if(firmaVal===null) return;
  const paseOpts=['na','pend'];
  const contOpts=['done','pend'];
  const paseVal=prompt(`Pase en vivo (na / fecha realizada / pend):`,c.pase);
  if(paseVal===null) return;
  const contVal=prompt(`Contenido firma (done / pend):`,c.contenido);
  if(contVal===null) return;
  confirmed[i].firma=firmaVal.trim();
  confirmed[i].pase=paseVal.trim();
  confirmed[i].contenido=contVal.trim();
  saveToStorage();
  updateMetrics();
  renderConfirmed();
}
